...
export abstract class MediaManager {
    ...
    async writeStreamToTempDirectory(stream: ReadStream, fileName: string): Promise<string> {
        if (!this.tempDir) {
            return Promise.reject(new Error('Could nto handle upload when no temp directory exists'))
        }
