...
export const getTokenTypeRegex = (type: CookieType, clientId: string): RegExp => {
    return new RegExp(`CognitoIdentityServiceProvider\\.${clientId}\\.[0-9a-z\\-_A-Z]+\\.${type}`)
}

export const getTokenTypeFromRequest = (cookiesData: Cookies, type: CookieType, clientId: string): string | null => {
...
export const keepTokensFresh = async (
    ...
    if (idToken) {
        ...
        if(isExpired) {
            ...
            if (!refreshToken) {
                log.err('Token was expired but the refresh token could not be found in the request cookies');
                return;
            }
